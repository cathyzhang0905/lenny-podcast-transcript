...
- ✅ **多频道扩展** + 可配置文件名模板
- ✅ **Agent 工具友好**:Claude Code Plugin / Codex CLI / Cursor / Aider 都能直接调用
...
本仓库支持三种 AI coding tool 工作流。

### Claude Code(`/plugin install` 一行装)
...
### OpenAI Codex CLI / Cursor / Aider(读 `AGENTS.md`)

```bash
git clone https://github.com/cathyzhang0905/lenny-podcast-transcript.git
cd lenny-podcast-transcript
python3 -m venv .venv && source .venv/bin/activate && pip install -r requirements.txt
export AI_API_KEY=sk-...  # 任何 OpenAI 兼容 provider

# 在仓库目录里启动 agent
codex          # 或 cursor / aider
# 之后直接说:
> Translate this Lenny's Podcast video to Chinese: https://www.youtube.com/watch?v=...
```

Agent 会读 [AGENTS.md](AGENTS.md) 拿到 trigger 条件 / 调用方式 / 错误处理,自动跑 `lenny_transcript.py`。

### 任何 LLM CLI(纯命令行)

```bash
git clone ... && cd lenny-podcast-transcript
python3 -m venv .venv && source .venv/bin/activate && pip install -r requirements.txt
export AI_API_KEY=sk-...
python lenny_transcript.py --url "https://www.youtube.com/watch?v=..." --lang zh
```

> ⚠️ **注意**:Plugin 和 AGENTS.md 都不会自动装 Python 依赖,你仍需手动 `pip install -r requirements.txt` + `export AI_API_KEY`(参考"快速开始")。
